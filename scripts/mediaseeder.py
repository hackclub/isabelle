...
def upload_to_cdn(url: str) -> str:
    j = json.dumps([url])
    res = requests.post("https://cdn.hackclub.com/api/v3/new", data=j,headers={"Authorization":"Bearer beans","Content-Type":"application/json"}).json()
    cdn_url = res.get("files")[0].get("deployedUrl")
